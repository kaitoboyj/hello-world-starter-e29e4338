...
import { withSolanaConnection, COVALENT_CHAIN_NAMES, covalentFetch } from '@/config/rpcEndpoints';
...
async function fetchEvmPriceFromCovalent(address: string, chainId: number): Promise<number> {
  ...
  try {
    const d = await covalentFetch(
      (key) =>
        `https://api.covalenthq.com/v1/pricing/historical_by_addresses_v2/${chainName}/USD/${address}/?key=${key}`
    );
...
export interface TokenMetadataResult {
  token: Token | null;
  source: 'jupiter' | 'moralis' | 'chain' | 'pumpfun' | 'dexscreener' | 'none';
  error?: string;
}
...
/**
 * Fetch full token metadata via DexScreener for ANY chain (EVM + Solana + others).
 * Includes logo, price, market cap, links — whatever DexScreener has indexed.
 */
export const getTokenFromDexScreener = async (
  address: string,
  chainId?: string
): Promise<Token | null> => {
  const t = await detectDexToken(address, chainId);
  if (!t) return null;
  return {
    address: t.address,
    symbol: t.symbol || 'UNK',
    name: t.name || 'Unknown Token',
    decimals: 0, // DexScreener does not expose decimals; on-chain fallback fills this for Solana
    logoURI: t.logoURI,
    price: t.priceUsd,
...
/**
 * Fetch token metadata from Pump.fun API (works for Pump.fun tokens)
 */
export const getTokenFromPumpFun = async (mintAddress: string): Promise<Token | null> => {
  try {
    ...
    if (data && data.mint) {
      return {
        ...
        logoURI: data.image_uri || data.uri || undefined,
      };
    }
...
export const getTokenMetadata = async (mintAddress: string): Promise<TokenMetadataResult> => {
  ...
  // If it's a Pump.fun token, try Pump.fun API first
  if (isPumpFunToken(mintAddress)) {
    console.log('Trying Pump.fun API for:', mintAddress);
    const pumpFunToken = await getTokenFromPumpFun(mintAddress);
    if (pumpFunToken) {
      // Enrich with price data
      const priceData = await getTokenPrice(mintAddress);
      if (priceData) {
        pumpFunToken.price = priceData.price;
        pumpFunToken.priceChange24h = priceData.priceChange24h;
      }
      return { token: pumpFunToken, source: 'pumpfun' };
    }
  }
  ...
  if (chainToken) {
    ...
    // If no logo yet, try DexScreener as final enrichment.
    if (!chainToken.logoURI) {
  ...
  // Last resort: DexScreener (any chain).
  const dexToken = await getTokenFromDexScreener(mintAddress);
  if (dexToken) return { token: dexToken, source: 'dexscreener' };
